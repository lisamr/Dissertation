...
# 
# # finding the optimum fixed structure - round 1
# ocn.lme1 <- lme(log(Other_CN) ~ Shrubs+ PNG +  AIF +(Crypto +
#                                    SOIL_Ca_kg_ha
#                                  +SOIL_Mg_kg_ha+SOIL_OM_pct+SOIL_TN_pct+SOIL_CN)* ppt,
#                 random = ~1|Site_number,
#                 method = "ML",
...
# validation
ocn.lmef <- lme(log(Other_CN) ~ Shrubs+ PNG +  AIF +
                  (Crypto + SOIL_Ca_kg_ha + SOIL_Mg_kg_ha + SOIL_OM_pct + 
                     SOIL_TN_pct + SOIL_CN)* ppt, 
                random = ~1|Site_number,
                method = "REML",
                control = ctrl,
                data = ocn.df)
...
# 
# # finding the optimum fixed structure
# pc.lme1 <- lme(Poa_TC_pct ~ AIF +Litter + SOIL_OM_pct+Crypto  +SOIL_TN_pct+ Rock+ PNG + SOIL_SurSo4_kg_ha
#                + (PNF +  AIG)* ppt,
#                random = ~1|Site_number,
#                method = "ML",
#                control = ctrl,
...
# validation
pc.lmef <- lme(Poa_TC_pct ~ AIF + Litter + SOIL_OM_pct + Crypto + SOIL_TN_pct +
...
sca.df <- sca.df[!is.na(all$Other_TC_pct),]
sca.df <- sca.df[!is.na(sca.df$Bromus_TN_pct),]
sca.df <- sca.df[!is.na(sca.df$Litter_TC_pct),]
# optimum fixed
sca.lme1 <- lme(log(SOIL_Ca_kg_ha) ~ PNG + (Crypto + ANF + AIF +
                                        Shrubs+Bromus_TC_pct+
                                        Other_TC_pct+Other_CN+
                                        Litter_TN_pct+Litter_TC_pct)* ppt,
                random = ~1 + Year|Site_number,
                method = "ML",
                data = sca.df,
                control = ctrl)
...
# Validation
sca.lmef <- lme(log(log(SOIL_Ca_kg_ha)) ~  PNG + (Crypto + ANF + AIF +
                                               Shrubs+Bromus_TC_pct+
                                               Other_TC_pct+Other_CN+
                                               Litter_TN_pct+Litter_TC_pct)* ppt, 
                random = ~1 + Year|Site_number, 
                method = "REML", 
                data = sca.df[sca.df$SOIL_Ca_kg_ha<10000,],
                control = ctrl)
...
# # finding the optimum fixed structure
# 
# sn.lme1 <- lme(SOIL_TN_pct ~ Rock + PNG + ANF + Litter + (Crypto + AIG + AIF +
#                                 Shrubs+Bromus_TC_pct+
#                                 Other_TC_pct+
#                                 Poa_TC_pct+Litter_TC_pct)* ppt,
#                random = ~1 + Year|Site_number,
#                method = "ML",
#                #weights = vp,
#                control = ctrl,
...
#validation
sn.lmef <- lme(SOIL_TN_pct ~ Rock + PNG + ANF + Litter + (Crypto + AIG + AIF +
                                              Shrubs+Bromus_TC_pct+
                                              Other_TC_pct+
                                              Poa_TC_pct+Litter_TC_pct)* ppt, 
               random = ~1 + Year|Site_number,
...
# 
# 
scn.df <- scn.df[!is.na(scn.df$Poa_TC_pct),]
scn.df <- scn.df[!is.na(scn.df$Litter_TC_pct),]

# scn.lme1 <- lme(log(SOIL_CN) ~  Rock + Litter_TC_pct + Poa_TC_pct+
#                   (PNG + PNF + AIG)* ppt,
#                random = ~1|Site_number,
#                method = "ML",
#                #weights = vp,
...
#validation
scn.lmef <- lme(log(SOIL_CN) ~ Rock + Litter_TC_pct + Poa_TC_pct+
                  (PNG + PNF + AIG)* ppt, 
                random = ~1 + Year|Site_number,
                method = "REML",
                control = ctrl,
