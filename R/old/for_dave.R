...
library(agricolae) # kruskal wallis
...
square_cor(pred,resp) #### veg correlated with soil/plant
tri_cor(resp) #### just the soil and plants in a triangle matrix

#### kw tests ####
# Right now this is going off the subplot level data, and I think it should
# properly be a friedman test, but I kept getting some error that 
# I couldn't figure out so this is it for now. The patterns are all pretty much
# the same as before I also changed it to display the medians instead of the
# means
...
# of course, it is totally frowned upon to do it like this by many people, and about half 
# of these models are basically worthless (especially the lm)
# but next I'll do it following the process recommended by zuur (2009)
# Some of the early parts of zuur's workflow from that book could be looped through with
...
set_theme(base=theme_classic()) # my preferred minimalist theme
...
for(i in 1:length(lmer.models)){
  if(class(lmer.models[[i]]$model) == "lmerMod"){
    plots[[i]] <- plot_model(lmer.models[[i]]$model, 
                     type = "est", 
                     title = paste(names(lmer.models)[i], class(lmer.models[[i]]$model)),
                     colors = "bw",
                     sort.est = TRUE,
                     show.values = TRUE,
                     show.p = TRUE)}
  if(class(lmer.models[[i]]$model) == "lm"){
    ...
    plots[[i]] <- sjp.lm(lmer.models[[i]]$model, 
                     type = "lm", 
...
# now just call from the plots list to check out each plot

#### visualizing the interactions ####

all$Precip_bin <- ifelse(all$ppt > 430, "Above Avg", "Below Avg")

#### Litter_TN ###

all$LTN_fit <- predict(lmer.models[[6]]$model, na.action = na.exclude)

p=ggplot(all, aes(x=PNG, y=Litter_TN_pct, shape=Precip_bin, size = ppt))+
  geom_point()+
  scale_shape_manual(values=c(1,16))+
  geom_smooth(aes(x = PNG, y = LTN_fit, color = Precip_bin), method="lm")

cowplot::plot_grid(plots[[6]], p)
...
# general parameters and functions to use below ####
...
densityplot((log(1 + ss.df$SOIL_SurSo4_kg_ha))) # skewed biomodal with some possible outliers
...
#coef_plots[[i]][[2]] <- viz_int(model_list[[i]])
}
