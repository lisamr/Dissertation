...
post_mod <- 
  ...
  mutate(aov = map(model, anova, ddf="Kenward-Roger"),
         ...
         # full_on_int = map(model, emmeans, contr = "tukey",
         #                     specs = ~invasion_stage:year),
         year_by_stage = map(model, emmeans, contr = "tukey",
                             specs = ~year|invasion_stage)) 

plot_the_stuff<- function(x, version, post_mod) {
  gc()
  ...
  if(version == "year"){
    ...
    # emmeans_sy<-   pluck( testmod$stage_by_year, 1)$emmeans%>%
    #   multcomp::cld(Letters=letters, decreasing=FALSE) %>% 
    #   as.data.frame %>%
    #   mutate(.group = str_to_upper(.group)) %>%
    #   left_join(dplyr::select(emmeans_y, invasion_stage, year, grp = .group)) %>%
    ...
    emmeans_s<-
     ...
     if(mean(dat$value) >3000) p <- p + scale_y_continuous(labels = scales::label_number_si())
     if(int_p<0.05) {p<-p +
       geom_label(data = emmeans_y%>%
                  mutate(year = ifelse(year == "2016", "'16", "'13")),
                nudge_y =nudge,#alpha=0.5,
     ...
     if(int_p >= 0.05 & inv_p < 0.05){
       p<- p+ geom_label(data = emmeans_s#%>%
                  ...
                  label.size=unit(0.00, "lines"),
                  label.padding = unit(0.01,"lines"),
                  aes(label=emmeans_s$.group,#color=invasion_stage,
                      x=1.5, y=UCL))
     }
  ...
  # if(version == "invasion_stage"){
  #   
    # emmeans_s<-
    #   pluck(testmod$stage_by_year, 1)$emmeans%>%
    #   multcomp::cld(Letters=letters, decreasing=FALSE) %>%
    #   as.data.frame
  #   return(
  # ggplot(dat) +
  #   geom_jitter(aes(x=invasion_stage, y=value, color = invasion_stage),
  #               alpha=0.5, width = 0.2, shape=1) +
  #   geom_errorbar(data=emmeans_s, width=0.4,aes(x=invasion_stage,
  #                                   y=emmean,
  #                                   ymin=lower.CL, 
  #                                   ymax=upper.CL,
  #                                   color = invasion_stage)) +
  #   geom_point(data = emmeans_s, 
  #              aes(x=invasion_stage, y=emmean, color = invasion_stage),
  #              size=3, shape=18)+
  #   geom_label(data = emmeans_s, nudge_y =nudge,
  #              alpha=0.25,
  #             size=4,fontface="bold",color = "black",
  #             label.size=unit(0.00, "lines"),
  #             label.padding = unit(0.01,"lines"),
  #             aes(label=emmeans_s$.group,#color=invasion_stage,
  #                 x=invasion_stage, y=upper.CL))+
  #   scale_color_manual(name = "Invasion Stage", 
  #                      values=site_colors)+
  #   scale_y_continuous(labels = scales::label_number())+
  #   ylab(lut_variables[dat$response])+
...
#test plots
plot_the_stuff(x="soil_n_kg_ha", version="year", post_mod=post_mod)
plot_the_stuff(x="Litter_TC_pct", version="year", post_mod=post_mod)

# plant_vars_inv <- resp[1:9] %>%
#   lapply(plot_the_stuff, version="invasion_stage", post_mod = post_mod)%>%
#   ggarrange(plotlist = ., nrow=3, ncol=3, common.legend = TRUE, labels = plot_labs) +
#   ggsave("figures/tidy_plantvars_inv.png",height=8, width=10)

plant_vars_y <- resp[1:9] %>%
  lapply(plot_the_stuff, version="year", post_mod = post_mod)%>%
  ggarrange(plotlist = ., nrow=3, ncol=3, labels = plot_labs) +
  ggsave("figures/tidy_plantvars_year.png",height=8, width=10)

soil_vars_y <- resp[10:18] %>%
  lapply(plot_the_stuff, version="year", post_mod = post_mod)%>%
  ggarrange(plotlist = ., nrow=3, ncol=3, labels = plot_labs) +
  ggsave("figures/tidy_soilvars_year.png",height=8, width=10)

# soil_vars_inv <- resp[10:18] %>%
#   lapply(plot_the_stuff, version="invasion_stage", post_mod = post_mod)%>%
#   ggarrange(plotlist = ., nrow=3, ncol=3, common.legend = TRUE, labels = plot_labs) +
#   ggsave("figures/tidy_soilvars_inv.png",height=8, width=10)
